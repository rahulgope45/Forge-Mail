...
export const createMailJob = async ({
    userId,
    to,
    subject,
    body,
    sendAt
}: CreateMailJobParams): Promise<CreateMailJobResult> => {
    ...
    const mailJob = await Promise.all(
        recipient.map((recipient) => prisma.mailJob.create({
            data: {
                userId,
                to: recipient,
                subject,
